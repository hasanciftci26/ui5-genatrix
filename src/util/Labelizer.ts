 ...
 */
export default class Labelizer extends BaseObject {
    public static run(propertyName: string) {
    ...
    private static getPropertyCasing(propertyName: string) {
    ...
    private static isConstantCase(propertyName: string) {
        const constantCase = /^[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)*$/;
        return constantCase.test(propertyName);
    }

    private static isSnakeCase(propertyName: string) {
        const snakeCase = /^[a-z][a-z0-9]*(?:_[a-z0-9]+)*$/;
        return snakeCase.test(propertyName);
    }

    private static isKebabCase(propertyName: string) {
        const kebabCase = /^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/;
        return kebabCase.test(propertyName);
    }

    private static isCamelCase(propertyName: string) {
        const camelCase = /^[a-z][a-zA-Z0-9]*$/;
        return camelCase.test(propertyName);
    }

    private static isPascalCase(propertyName: string) {
        const pascalCase = /^[A-Z][a-zA-Z0-9]*$/;
        return pascalCase.test(propertyName);
    }

    private static labelizeConstantCase(propertyName: string) {
    ...
    private static labelizeSnakeCase(propertyName: string) {
        const parts = propertyName.split("_");
        return parts.map(word => word.length > 0 ? word.charAt(0).toUpperCase() + word.slice(1) : word).join(" ");
    }

    private static labelizeKebabCase(propertyName: string) {
        const parts = propertyName.split("-");
        return parts.map(word => word.length > 0 ? word.charAt(0).toUpperCase() + word.slice(1) : word).join(" ");
    }

    private static labelizeCamelCase(propertyName: string) {
    ...
    private static labelizePascalCase(propertyName: string) {
        const parts = propertyName.match(/([A-Z]{2,}(?=[A-Z][a-z]|[0-9]|$))|([A-Z]?[a-z]+[0-9]*[a-z]*)|([0-9]+)/g);
